...
vec4f Matrix4x4::row(unsigned int index)
{
	return vec4f(m[index][0], m[index][1], m[index][2], m[index][3]);
}

vec4f Matrix4x4::col(unsigned int index)
{
	return vec4f(m[0][index], m[1][index], m[2][index], m[3][index]);
}
...
Matrix4x4 Matrix4x4::perspectiveQT(float fovY, float aspect, float near, float far)
{
	Matrix4x4 m;
	qreal radians = (fovY / 2.0f) * M_PI / 180.0f;
	...
	m.m[0][2] = 0.0f;
	m.m[1][2] = 0.0f;
	m.m[2][2] = -(near + far) / clip;
	m.m[3][2] = -(2.0f * near * far) / clip;
	m.m[0][3] = 0.0f;
	m.m[1][3] = 0.0f;
	m.m[2][3] = -1.0f;
	m.m[3][3] = 0.0f;
...
{
	QByteArray bits;
	if (m.m_type.identiy & true)
		bits += "Identity,";
	if (m.m_type.translate & true)
		bits += "Translate,";
	if (m.m_type.scale & true)
		bits += "Scale,";
	if (m.m_type.rotation & true)
		bits += "Rotation,";
	if (bits.size() > 0)
		bits = bits.left(bits.size() - 1);

	d.nospace() << "Matrix4x4[" << bits.constData() << endl
		<< qSetFieldWidth(10)
